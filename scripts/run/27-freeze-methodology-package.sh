...
cat > "$FREEZE_DIR/FINAL_PACKAGE_METADATA.json" <<EOF
{
  ...
  "sha256_policy": "SHA256SUMS is excluded from its own checksum manifest."
}
EOF
...
Integrity policy: `SHA256SUMS` is intentionally excluded from its own manifest.
EOF
...
(
  cd "$FREEZE_DIR"
  # IMPORTANT:
  # Exclude SHA256SUMS from its own checksum manifest.
  # Including it causes a self-referential checksum mismatch during validation.
  find . -type f ! -name 'SHA256SUMS' -print0 | sort -z | xargs -0 sha256sum > SHA256SUMS
  sha256sum -c SHA256SUMS >/tmp/final_methodology_package_sha256_check.log
)

tar -czf "$ARCHIVE" -C "$(dirname "$FREEZE_DIR")" "$(basename "$FREEZE_DIR")"

line
echo "[SUMMARY] Final Methodology Package Freeze v2"
line
...
ok "SHA256SUMS generado y validado sin incluirse a sí mismo" "SHA256SUMS generated and validated without including itself"
